...
/**
 * 包含两个ScrollView的容器
 * 实现：在第一个ScrollView滚动到最底下时会有提示，继续拖动才能浏览图片。仿照这个效果写一个出来并不难，
 * 只要定义一个Layout管理两个ScrollView就行了，当第一个ScrollView滑到底部时，再次向上滑动进入第二个ScrollView。
 */
public class ScrollViewContainer extends RelativeLayout {
    ...
    /**
     * 第二个页面包含有webView时，传入次webview，用于解决webview与滑动的冲突
     */
    private WebView mWebView;

    //处理手指离开屏幕后的自动滚动，棒棒的写法
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                if (mWebView != null) {
                    //滑动到第二页的顶部 处理事件
                    if (mWebView.getScrollY() == 0 && mCurrentViewIndex == 1) {
                        canPullDown = true;
                    } else {
                        canPullDown = false;
                    }
     ...
     * 设置第二个view为webview时
     */
    public void setBottomView(WebView mWebView) {
    ...
    private OnTouchListener bottomViewTouchListener = new OnTouchListener() {
        ...
        @Override
        public boolean onTouch(View v, MotionEvent event) {
            LinearLayout sv = (LinearLayout) v;
            //滑动到第二页的顶部 处理事件
    ...
    public interface OnUpPullHintListener {
        ...
        public static final int STATE_DWON = 2; //滚回提示
         ...
         void onUpPullHintListener(int state);
    }
