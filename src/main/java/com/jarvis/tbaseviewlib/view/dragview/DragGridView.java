...
public class DragGridView extends GridView {
		...
		if (isDrag && mDragImageView != null) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (dragListener != null) {
					dragListener.dragMove(mDragPosition, ev.getRawX(), ev.getRawY()-mStatusHeight);
				}
				// 拖动item
			...
			case MotionEvent.ACTION_UP:
				if (dragListener != null) {
					dragListener.dragUp(mDragPosition, ev.getRawX(), ev.getRawY()-mStatusHeight);
				}
				onStopDrag();
				isDrag = false;
				break;
	...
	private void onSwapItem(int moveX, int moveY) {
		...
		if (tempPosition != mDragPosition && tempPosition != AdapterView.INVALID_POSITION && mAnimationEnd&&isMoveItem) {
			try {
				// ----------------做数据交换----------------
				if (mDragAdapter.getData() != null) {
					List<Object> list = (List<Object>) mDragAdapter.getData();
					Object tempData = list.get(mDragPosition);
					if (mDragPosition < tempPosition) {
						for (int i = mDragPosition; i < tempPosition; i++) {
							Collections.swap(list, i, i + 1);
						}
					} else if (mDragPosition > tempPosition) {
						for (int i = mDragPosition; i > tempPosition; i--) {
							Collections.swap(list, i, i - 1);
						}
					}
					list.set(tempPosition, tempData);
				} else {
					mDragAdapter.reorderItems(mDragPosition, tempPosition);
				}
				// ----------------做数据交换End----------------
			} catch (Exception e) {
				// 如果出现数据转换异常，那么就必须在adapter的reorderItems()方法中去实现数据交换
				Log.e("Exception", e.toString() + "!!!you must do it in Method of reorderItems");
				mDragAdapter.reorderItems(mDragPosition, tempPosition);
			}
	 ...
	 * @param isMoveItem
	 */
	public void setMoveItem(boolean isMoveItem) {
