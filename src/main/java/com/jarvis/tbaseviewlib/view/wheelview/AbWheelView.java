...
//	private Context mContext = null;
	...
	/** Items text color. */
	private static final int ITEMS_TEXT_COLOR = 0xFFABABAB;
...
//		}
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			//一个字符宽度
			float textWidth = (int)getStringWidth("0", labelPaint);
			//不算lable的宽度
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int)getStringWidth(label, labelPaint);
...
//	}
	...
	public void setValueTextSize(int textSize) {
		this.valueTextSize = resizeTextSize(screenWidth,screenHeight, textSize);
	...
	public void setLabelTextSize(int labelTextSize) {
		this.labelTextSize = resizeTextSize(screenWidth,screenHeight, labelTextSize);
	}
     ...
     */
	private float getStringWidth(String str,TextPaint paint) {
     ...
     * @return the int
     */
    private int resizeTextSize(int screenWidth,int screenHeight,int textSize){
        float ratio =  1;
        try {
